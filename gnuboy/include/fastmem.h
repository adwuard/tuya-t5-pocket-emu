...
static int __attribute__((unused)) readw(int a)
{
	if ((a+1) & 0xfff)
	{
		byte *p = mbc.rmap[a>>12];
...
static void __attribute__((unused)) writew(int a, int w)
{
	if ((a+1) & 0xfff)
	{
		byte *p = mbc.wmap[a>>12];
...
static byte __attribute__((unused)) readhi(int a)
{
	return readb(a | 0xff00);
}

static void __attribute__((unused)) writehi(int a, byte b)
{
	writeb(a | 0xff00, b);
}
